...
func Tar(tarName string, paths []string) (err error) {
	...
	if strings.HasSuffix(tarName, ".gz") || strings.HasSuffix(tarName, ".tgz") || strings.HasSuffix(tarName, ".gzip") {
...
func Untar(tarName, xpath string) (err error) {
	...
	if strings.HasSuffix(tarName, ".bz2") {
		bz := bzip2.NewReader(tarFile)
		// defer bz.Close()
		tr = tar.NewReader(bz)
	}
